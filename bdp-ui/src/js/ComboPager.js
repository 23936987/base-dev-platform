...
_$.extend(_$.ComboPager,_$.ComboSearch, {
    ...
    _mutiItemSelect:function (val) {
        ...
        var ids = [];
        var names=[];
        if(isEmpty(_this.value) || isEmpty(_this.value["ids"])){
            ids = [];
            names=[];
        }else {
            ids = _this.value["ids"].split(",");
            names = _this.value["names"].split(",");
        }
    ...
    setValue:function (value,change) {
        ...
        if (isEmpty(value)) {
            ...
            if (_this._isEdit()) {
                if (isEmpty(_this.data) || _this.data.length == 0) {
                    _this.value = value;
                    return;
                }
                if (!multiple) {
