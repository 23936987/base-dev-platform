...
public class CrudScene implements IScene {
    private static Logger logger = LoggerFactory.getLogger(CrudScene.class);
    protected Map<String,Object> data = new HashMap<String, Object>();
    private CrudConfig config;

    public CrudScene(CrudConfig config){
        try {
        	this.config = config;
            _init(config);
        } catch (Exception e) {
    ...
    private void _init(CrudConfig config) throws Exception {
        //读取配置文件
	...
	public void makeFile(String ftlPath,String filePath) throws Exception {
		...
		cfg.setClassForTemplateLoading(CrudScene.class, "/template/"+scene+"/ftl/");
